...
function [vx,vy,energylist]=SIFTflowc2f(im1,im2,SIFTflowpara,I1,I2,isdisplay,Segmentation)
...
if isfield(SIFTflowpara,'median')
    isMF=SIFTflowpara.median;
else
    isMF=false;
end
...
%     pyrd(i).G=imresize(imfilter(pyrd(i-1).G,fspecial('gaussian',5,0.67),'same','replicate'),0.5,'bicubic');
    pyrd(i).im1=imresize(pyrd(i-1).im1,0.5,'bilinear','Antialiasing',false);
    pyrd(i).im2=imresize(pyrd(i-1).im2,0.5,'bilinear','Antialiasing',false);
    pyrd(i).G=imresize(pyrd(i-1).G,0.5,'bilinear');
...
%         winSizeY=decideWinSize(vy,wsize);
    ...
    if i==nlevels
        if IsSegmentation
            ...
            [flow,foo]=mexDiscreteFlow(Im1,Im2,[alpha,d,gamma*2^(i-1),nTopIterations,2,topwsize],vx,vy,winSizeX,winSizeY,G1,G1);
...
%         end
        ...
        if IsSegmentation
            ...
            [flow,foo]=mexDiscreteFlow(Im1,Im2,[alpha,d,gamma*2^(i-1),nIterationArray(i),nlevels-i,wsize],vx,vy,winSizeX,winSizeY,G1,G1);
...
%         [flow,foo]=mexDiscreteFlow(Im1,Im2,[alpha,d,gamma*2^(i-1),nIterationArray(i),0,wsize],vx,vy,winSizeX,winSizeY);
    ...
    vy=flow(:,:,2);
    if isMF
        vx = medfilt2(vx, [5,5], 'symmetric');
        vy = medfilt2(vy, [5,5], 'symmetric');
    end
    if isdisplay
        fprintf('done!\n');
    end
